...
def _train(dataset: Path, model_path: Path) -> tuple[ActionRecognizer, int]:
    records = [record for record in discover_sequences(dataset) if record.annotation is not None]
    if len(records) < 6:
    ...
    for index, record in enumerate(records, 1):
        ...
        labels.append(record.annotation.class_id)  # type: ignore[union-attr]
...
def command_predict(args: argparse.Namespace) -> None:
    ...
    result = {"label": prediction.label, "class_id": prediction.class_id,
...
def command_experiment(args: argparse.Namespace) -> None:
    result = run_experiment(
        args.dataset, args.output, seed=args.seed,
        strict_proposal=not args.allow_nonstandard,
        qualitative_videos=not args.no_videos,
    )
    summary = {
        "model": str(result.model_path), "train_sequences": result.train_count,
        "test_sequences": result.test_count,
        "metrics": {key: value for key, value in result.metrics.items() if key != "results"},
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="hri-action", description="40-frame HRI action recognition MVP")
    commands = parser.add_subparsers(dest="command", required=True)
    train = commands.add_parser("train", help="Train from labeled sequence folders")
    train.add_argument("dataset"); train.add_argument("--model", default="artifacts/action_model.joblib")
    train.set_defaults(func=command_train)
    predict = commands.add_parser("predict", help="Classify and annotate one sequence folder")
    predict.add_argument("sequence"); predict.add_argument("--model", required=True)
    ...
    test.add_argument("dataset"); test.add_argument("--model", required=True)
    ...
    validate.add_argument("--allow-nonstandard", action="store_true", help="Allow datasets other than the exact 72-sequence design")
    validate.set_defaults(func=command_validate)
    experiment = commands.add_parser("experiment", help="Run validation, stratified training, evaluation and artifact generation")
    ...
    demo.add_argument("--output", default="artifacts/demo"); demo.add_argument("--fresh", action="store_true")
